...
RSpec.describe Gemstash::Storage::LocalStorage do
  before do
    @folder = Dir.mktmpdir
  end
  after do
  ...
  it "builds with a valid folder" do
    expect(described_class.new(@folder)).not_to be_nil
  end
  ...
  context "with a valid storage" do
    let(:storage) { described_class.new(@folder) }
    ...
    it "auto sets gemstash version property, even when properties not saved" do
      ...
      expect(resource.properties).to eq(gemstash_resource_version: Gemstash::Storage::VERSION)
    end
    ...
    it "won't load a resource that is at a larger version than our current version" do
      ...
      expect { storage.resource("42").content(:content) }.to raise_error(Gemstash::Storage::LocalStorage::Resource::VersionTooNew, /42/)
    end
    ...
    context "with a simple resource" do
      ...
      it "can also save properties" do
        ...
        expect(resource.properties).to eq("content-type" => "octet/stream",
                                          gemstash_resource_version: Gemstash::Storage::VERSION)
      end
    ...
    context "with a previously stored resource" do
      ...
      it "can have properties updated" do
        ...
        expect(storage.resource(resource_id).properties).
          to eq(key: "value", other: :value, gemstash_resource_version: Gemstash::Storage::VERSION)
        resource = storage.resource(resource_id)
        resource.update_properties(key: "new", new: 42)
        expect(storage.resource(resource_id).properties).
          to eq(key: "new", other: :value, new: 42, gemstash_resource_version: Gemstash::Storage::VERSION)
      end
    ...
    context "storing multiple files in one resource" do
      ...
      it "can be done in 2 saves with separate properties defined" do
        ...
        expect(resource.properties).to eq(foo: "bar", bar: "baz", gemstash_resource_version: Gemstash::Storage::VERSION)
      ...
      it "can be done in 2 saves with nil properties defined on second" do
        ...
        expect(resource.properties).to eq(foo: "bar", gemstash_resource_version: Gemstash::Storage::VERSION)
